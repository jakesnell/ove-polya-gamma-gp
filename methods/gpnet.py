...
class GPNet(MetaTemplate):
    ...
    def _reset_variational(self):
        ...
        for idx, param in enumerate(self.gp_layer.variational_parameters()):
            if idx == 0:
                ...
                raise ValueError(
                    "[ERROR] GPNet the variational_parameters at index>1 should not exist!"
                )
    ...
    def _reset_parameters(self):
        if self.leghtscale_list is None:
            ...
            for idx, single_model in enumerate(self.model.models):
                self.leghtscale_list.append(
                    single_model.covar_module.base_kernel.lengthscale.clone().detach()
                )
                self.noise_list.append(single_model.likelihood.noise.clone().detach())
                self.outputscale_list.append(
                    single_model.covar_module.outputscale.clone().detach()
                )
        else:
            for idx, single_model in enumerate(self.model.models):
                single_model.covar_module.base_kernel.lengthscale = (
                    self.leghtscale_list[idx].clone().detach()
                )  # .requires_grad_(True)
                single_model.likelihood.noise = self.noise_list[idx].clone().detach()
                single_model.covar_module.outputscale = (
                    self.outputscale_list[idx].clone().detach()
                )
    ...
    def train_loop(self, epoch, train_loader, optimizer, print_freq=10):
        ...
        for i, (x, _) in enumerate(train_loader):
            ...
            x_all = (
                x.contiguous()
                .view(self.n_way * (self.n_support + self.n_query), *x.size()[2:])
                .cuda()
            )
            y_all = Variable(
                torch.from_numpy(
                    np.repeat(range(self.n_way), self.n_query + self.n_support)
                ).cuda()
            )
            x_support = (
                x[:, : self.n_support, :, :, :]
            ...
            z_train = self.feature_extractor.forward(
                x_train
            )  # .detach() #[340, 64] #TODO enable detach()
            train_list = [z_train] * self.n_way
            lenghtscale = 0.0
            noise = 0.0
            outputscale = 0.0
            for idx, single_model in enumerate(self.model.models):
                single_model.set_train_data(
                    inputs=z_train, targets=target_list[idx], strict=False
                )
                if single_model.covar_module.base_kernel.lengthscale is not None:
                    lenghtscale += (
                        single_model.covar_module.base_kernel.lengthscale.mean()
                        .cpu()
                        .detach()
                        .numpy()
                        .squeeze()
                ...
                if single_model.covar_module.outputscale is not None:
                    outputscale += (
                        single_model.covar_module.outputscale.cpu()
                        .detach()
                        .numpy()
                        .squeeze()
                    )
            ...
            with torch.no_grad():
                ...
                predictions = self.likelihood(
                    *self.model(*z_support_list)
                )  # return 20 MultiGaussian Distributions
                predictions_list = list()
                for gaussian in predictions:
                    predictions_list.append(
                        torch.sigmoid(gaussian.mean).cpu().detach().numpy()
                    )
                y_pred = np.vstack(predictions_list).argmax(axis=0)  # [model, classes]
                accuracy_support = (
                    np.sum(y_pred == y_support) / float(len(y_support))
                ) * 100.0
                if self.writer is not None:
                    self.writer.add_scalar(
                        "GP_support_accuracy", accuracy_support, self.iteration
                    )
                z_query = self.feature_extractor.forward(x_query).detach()
                z_query_list = [z_query] * len(y_query)
                predictions = self.likelihood(
                    *self.model(*z_query_list)
                )  # return 20 MultiGaussian Distributions
                predictions_list = list()
                for gaussian in predictions:
                    predictions_list.append(
                ...
                y_pred = np.vstack(predictions_list).argmax(axis=0)  # [model, classes]
                accuracy_query = (
                    np.sum(y_pred == y_query) / float(len(y_query))
                ) * 100.0
                if self.writer is not None:
                    self.writer.add_scalar(
                        "GP_query_accuracy", accuracy_query, self.iteration
                    )

            if i % print_freq == 0:
                if self.writer is not None:
                    self.writer.add_histogram("z_support", z_support, self.iteration)
                print(
    ...
    def correct(self, x, N=0, laplace=False):
        ...
        with torch.no_grad(), gpytorch.settings.num_likelihood_samples(32):
            ...
            predictions = self.likelihood(
                *self.model(*z_query_list)
            )  # return n_way MultiGaussians
            predictions_list = list()
            logits = []
            for gaussian in predictions:
                logits.append(gaussian.mean.cpu().detach().numpy())
                predictions_list.append(
                    torch.sigmoid(gaussian.mean).cpu().detach().numpy()
                )
            logits = np.vstack(logits).T
            #logits = np.log(np.vstack(predictions_list).T)
            y_pred = np.vstack(predictions_list).argmax(axis=0)  # [model, classes]
            top1_correct = np.sum(y_pred == y_query)
            count_this = len(y_query)
        return float(top1_correct), count_this, avg_loss / float(N + 1e-10), logits

    def test_loop(self, test_loader, record=None, return_std=False, use_progress=None, return_stats=False):
        print_freq = 10
        correct = 0
        count = 0
        acc_all = []
        ...
        for i, (x, _) in enumerate(test_loader):
            ...
            if self.change_way:
                self.n_way = x.size(0)
            correct_this, count_this, loss_value, logits = self.correct(x)
            ...
            if i % 100 == 0:
                ...
                print(
                    "Test | Batch {:d}/{:d} | Loss {:f} | Acc {:f}".format(
                        i, len(test_loader), loss_value, acc_mean
                    )
                )
...
class ExactGPLayer(gpytorch.models.ExactGP):
    ...
    def __init__(self, train_x, train_y, likelihood, kernel="linear"):
        ...
        ## Polynomial (p=2)
            ...
            raise ValueError(
                "[ERROR] the kernel '" + str(kernel) + "' is not supported!"
            )
